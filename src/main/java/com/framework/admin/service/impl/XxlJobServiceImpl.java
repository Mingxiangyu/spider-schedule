...
import com.framework.admin.core.route.ExecutorRouteStrategyEnum;
...
import com.framework.admin.dao.XxlJobGroupDao;
import com.framework.admin.dao.XxlJobInfoDao;
import com.framework.admin.dao.XxlJobLogDao;
import com.framework.admin.dao.XxlJobLogGlueDao;
import com.framework.admin.dao.XxlJobLogReportDao;
import com.framework.admin.service.XxlJobService;
...
public class XxlJobServiceImpl implements XxlJobService {
  ...
  public ReturnT<String> start(int id) {
    ...
    // 如果该job的调度类型是 NONE ，则返回 “当前调度类型禁止启动”
    ...
    try {
      ...
      if (nextValidTime == null) {
        // 如果下一次执行时间为null 则意味调度类型不符合定时策略以及固定时间，直接返回调度类型非法
        return new ReturnT<String>(
            ReturnT.FAIL_CODE,
            (I18nUtil.getString("schedule_type") + I18nUtil.getString("system_unvalid")));
      }
    ...
    // 更新数据库中任务调度状态为运行，设置下次调度时间，后续即可被调度线程扫表时查询执行调度。

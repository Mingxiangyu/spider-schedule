 ...
 *
 * <p>a(*)、LFU(Least Frequently Used)：最不经常使用，频率/次数
 *
 * <p>b、LRU(Least Recently Used)：最近最久未使用，时间
 *
 * <p>算法思想：
 *
 * <p>构建一个作业和地址map jobid -> addressList
 *
 * <p>第一次随机的将任务所对应的执行器的注册地址编一个序列号
 *
 * <p>然后将执行器的注册地址按照从小到大进行排序
 *
 * <p>筛选过程找第一个序列号最小的作为下一次的路由地址
 *
 * <p>随后将当前选中的地址编号值+1
 *
 * <p>这样最终我们都会挑选编号最小的注册器地址作为下一个路由地址，既最不常使用的
 *

...
import com.framework.admin.core.complete.XxlJobCompleter;
...
public class JobCompleteHelper {
  ...
  // 用于callback 回调的线程池，客户端调用api/callback接口时会使用这个线程池
  private ThreadPoolExecutor callbackThreadPool = null;
  // 调度记录停留在 "运行中" 状态 超过10min，且对应执行器心跳注册 失败不在线，则将本地调度主动标记失败
        ...
        new ThreadPoolExecutor(
            ...
            new ThreadFactory() {
              ...
              public Thread newThread(Runnable r) {
                return new Thread(
                    r, "xxl-job, admin JobLosedMonitorHelper-callbackThreadPool-" + r.hashCode());
              }
            },
  ...
  private ReturnT<String> callback(HandleCallbackParam handleCallbackParam) {
    // valid log item
    XxlJobLog log =
        XxlJobAdminConfig.getAdminConfig().getXxlJobLogDao().load(handleCallbackParam.getLogId());
    if (log == null) {

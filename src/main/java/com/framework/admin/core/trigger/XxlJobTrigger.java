package com.framework.admin.core.trigger;
...
public class XxlJobTrigger {
   ...
   * 获取失败重试次数
   * 获取任务组（执行机器）
   * 判断是否需要分片广播（此处可将分片参数传入，每台机器执行对应的分片任务） processTrigger
   *
   * <p>获取阻塞策略 获取路由策略 获取分片参数 记录日志log-id 初始化触发器参数 初始化调用地址 runExecutor
   *
   * <p>rpc远程调用executorBiz.run(triggerParam)（触发执行器执行任务）
   *
   * <p>获取调用结果 记录日志trigger-info
   *
   * <p>作者：CoderJie 链接：https://juejin.cn/post/7069222244785848356 来源：稀土掘金
   */
   ...
   * <p>获取失败重试次数
   *
   * <p>根据jobGroup获取任务组（执行机器）
   *
   * <p>判断是否需要分片广播（此处可将分片参数传入，每台机器执行对应的分片任务）
   *
   * @param jobId
   * @param triggerType
  ...
  private static void processTrigger(
    ...
    // 2、init trigger-param 初始化trigger-param
